...
export class ImageUploadService {
  ...
  private static getOptionsConfig() {
    return {
      headers: {
        'authorization': JSON.parse(localStorage.getItem('yalladeals')).token,
    ...
    private static async sendToAPIFile(form: FormData): Promise<Array<string>> {
        return await axios
      .post(`${(process.env.BASE_URL || 'https://uae.yalladealz.com/api')}/upload/image`, form, this.getOptionsConfig())
      .then(data => {
